  ...
  try {
    inquirer.prompt([
      ...
      },
      {
        ...
        choices: (arg) => {
          ...
          try {
            ...
            const branchList = execSync('git ls-remote --heads').toString().split('\n')
            return branchList.map((item) => item.split('refs/heads/').pop()).filter((item) => item)
          } catch (e) {
            throw new Error(e)
          }
        }
      ...
      .then(async (answers) => {
        ...
        try {
          if (!dir) {
            execSync(`git pull origin ${branch || 'master'}`, { stdio: 'inherit' })
          } else {
            // 获取当前的系统
